...
Renderer 在冻结、暂停、循环以及桌面/屏保宿主切换时保留最后提交帧。普通覆盖暂停停止解码和帧调度，最多保留 30 秒的快速恢复窗口；重复暂停不能延长该窗口。关闭活动播放、屏保退出至静态桌面或已有低内存压力时，在静态画面提交后的约 250 毫秒触发资源释放，不再依赖系统内存不足才清理。

冻结边界按完整显示分辨率捕获 DirectComposition 表面，交由 DWM 保留画面；清理时释放 Media Engine 或内置解码器及双缓冲交换链，压缩失败时以 30 秒间隔重试。重复冻结直接确认已有画面，不重新加载视频。恢复时保留静态表面，创建新解码器和交换链、定位到记录时间并提交首帧，最后才切换视觉层。过期定时器在播放状态下无效。仅屏保模式在闲置前及唤醒后不保留 Renderer 路线；后台性能副本生成由独立任务策略管理。资源释放不会降低静态画面的分辨率。
